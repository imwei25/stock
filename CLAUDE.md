...
- **因子面板落盘缓存**:`data/factor_panels/<sig>/`,sig = (factors + codes + 日期范围 + preprocess + panel_version) 哈希;任一变化自动重算。`--refresh-factor-panel` 旁路。panel_version 在算子语义变更时 +1(v2 = 2026-06-12 `ops.correlation` 出口 ±inf→NaN + clip [-1,1];v1 缓存的 alpha_045 等含 inf 毒值,已整体失效)。
...
- **选因子无 in-sample 偏差**:`factors analyze --end-date` 把 selection 窗口截止在回测起点之前;analyze 与生产共用同一预处理/mask/标签口径,ic_ir 带 Newey-West 修正。当前 `reports/selection.json` = **top-30**(2026-06-13 v2 干净面板重选,改进轮6 从 top-20 扩到 top-30:sharpe +0.062),选自 ≤2024-05-20 窗口,评估期 2024-05-21 起 500 根。`selection.json` 已纳入版本控制(.gitignore 例外)。
...
- **预处理默认值经 A/B 定标**:winsorize+zscore 默认开,market_cap_neutralize 默认开(v2 干净面板上优势 +0.197 sharpe,12/16 胜),industry_neutralize / symmetric_orthogonalize 默认关。结论见 `docs/ab_validation_results.md`(2026-06-13 v2 面板整批重跑为权威结果,11 组方向性结论与 v1 一致无翻转)。
