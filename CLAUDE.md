...
- **`preprocess.{winsorize, zscore, industry_neutralize}`**(2026-06-06 Phase 1):截面预处理三步,winsorize 默认 `null`,zscore 与 industry_neutralize 默认 `false`(整体全关 → 向后兼容、cache sig 不变)。baked 进 `factor_panels/<sig>/` cache(sig 在 preprocess 非全关时把 cfg dump 进 hash);改 preprocess → 自动新 sig 重算。industry_neutralize 自动跳过 factor `types` 含 `"fundamental"` 的因子,保留 bank-low-PE 类行业内信号。⚠️ Phase 1 AB 验证 indecisive(`docs/ab_validation_results.md` P4-1),默认保持全关,见 Phase 1.5 follow-up
