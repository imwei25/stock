...
- **`preprocess.{winsorize, zscore, industry_neutralize, market_cap_neutralize, min_pool_size}`**(2026-06-06 Phase 1+1.5,2026-06-08 Phase 2):截面预处理流水线。`winsorize`/`zscore` 默认 `null`/`false`(已按 P4-1b 验证默认开启:+0.245 sharpe / +2.64% return on 4358-票训练池);`industry_neutralize` 默认 `false`(单成员细分行业会触发 silent demean-to-zero,全市场参照设计前不推荐开启)。**`market_cap_neutralize`**(Phase 2,默认 `false`):每日截面把因子对 log(总市值) 做 OLS 残差化,剥离规模暴露;mcap = `close × totalShare`(totalShare 取 `data/mcap_shares.parquet` 最新快照,静态广播,见 `scripts/pull_mcap_profit.py`)。需 caller 注入 log_mcap 面板(prepare_pool/cli/ab.runner 已 wire);无面板时静默跳过 + warning。与 industry_neutralize 同属 ④ 中性化步,均跳过 types 含 "fundamental" 的因子。`min_pool_size: int = 200` runtime guard,n_codes < 200 时四步全跳 + warning(避免小池子退化)。baked 进 `factor_panels/<sig>/` cache(preprocess 入 hash);改 preprocess → 自动新 sig 重算。AB 对比见 `ab_neutralize.yaml`(industry vs market_cap arm)。详见 `docs/ab_validation_results.md` P4-1b/P4-2 / `docs/superpowers/specs/2026-06-06-factor-preprocessing-phase1-design.md`.
