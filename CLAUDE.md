...
| `src/stockpool/ml/preprocess.py` | **截面预处理流水线** (winsorize / cs_zscore / industry_neutralize / **market_cap_neutralize**)。stateless 四函数 + `apply_preprocess_pipeline` 驱动 + `_is_all_off` short-circuit。`market_cap_neutralize_panel`(Phase 2)= 每日截面把因子对 log(总市值) 做 OLS 残差化(向量化,无 python 循环;size NaN 的票 pass-through,退化截面回退 demean)。baked 进 `build_factor_panel` cache,所有 callers 共享。industry/market_cap 两步均跳过 types 含 "fundamental" 的因子 |
...
- **`preprocess.{winsorize, zscore, industry_neutralize, market_cap_neutralize, min_pool_size}`**(2026-06-06 Phase 1+1.5,2026-06-08 Phase 2):截面预处理流水线。`winsorize`/`zscore` 默认 `null`/`false`(已按 P4-1b 验证默认开启:+0.245 sharpe / +2.64% return on 4358-票训练池);`industry_neutralize` 默认 `false`(单成员细分行业会触发 silent demean-to-zero,全市场参照设计前不推荐开启)。**`market_cap_neutralize`**(Phase 2,**默认 `true`**,P4-3 PASS):每日截面把因子对 log(总市值) 做 OLS 残差化,剥离规模暴露;mcap = `close × totalShare`(totalShare 取 `data/mcap_shares.parquet` 最新快照,静态广播,见 `scripts/pull_mcap_profit.py`)。需 caller 注入 log_mcap 面板(prepare_pool/cli/ab.runner 已 wire);**无 `data/mcap_shares.parquet` 时静默跳过 + warning**(所以缺数据不会报错,只是退化为不中性)。与 industry_neutralize 同属 ④ 中性化步,均跳过 types 含 "fundamental" 的因子。`min_pool_size: int = 200` runtime guard,n_codes < 200 时四步全跳 + warning(避免小池子退化)。baked 进 `factor_panels/<sig>/` cache(preprocess 入 hash);改 preprocess → 自动新 sig 重算。**A/B 结论**:P4-2 market_cap 全面优于 industry(Δsharpe +0.249);P4-3 market_cap 优于 no-neutralize base(Δsharpe +0.156,PASS)→ 默认开启。`industry_neutralize` 实测**有害**(与 selection 里的 `industry_relative_strength` 因子冲突),维持默认 false。对比配置 `ab_neutralize.yaml` / `ab_neutralize_confirm.yaml`。详见 `docs/ab_validation_results.md` P4-2/P4-3.
...
| `test_ml_preprocess.py` | 截面预处理 — 4 函数(winsorize / cs_zscore / industry_neutralize / **market_cap_neutralize**)+ `apply_preprocess_pipeline` 流水线(含 mcap 残差化 + fundamental 跳过 + missing-panel warning)+ `_is_all_off` short-circuit + size guard,31 个 case |
