...
python -m stockpool portfolio-backtest --config config.yaml [--refresh-scores] [--workers N]
...
| `src/stockpool/portfolio_ab/` | **Portfolio AB 子包** (PR-4): `config.py` (`PortfolioABConfig` / `PortfolioArmOverride` 仅允许覆盖 `strategy` 整段替换 + `portfolio_backtest` 字段级合并 / `load_portfolio_ab_config` / `build_effective_cfg` — content_hash 重算保证 score panel 缓存隔离) + `runner.py` (`ArmResult` / `ABResult` / `run_single_arm` / `run_portfolio_ab` — 共享 universe + sector_map + name_map,per-arm failure 隔离;**2026-06-27** score panel 三级复用:in-memory `score_memo`(`run_portfolio_ab` 串行跨 arm 共享,**覆盖 `--refresh-scores`**——磁盘 cache 被绕过时仍只算一次)→ 磁盘 `score_cache_key` parquet → 重算;命中 memo/磁盘时**跳过 `load_or_build_factor_panel` + `build_strategy`**。并行 arm 子进程无法共享 memo,仍靠磁盘 cache) + `report.py` (`render_portfolio_ab_report` — banner / 聚合指标 + Δ 表 / 双 arm 净值 overlay / per-stock 贡献分解 top 15 / 已交易 code 集合分析 only-A/only-B/both / 失败 arm 红 banner) |
| `src/stockpool/portfolio/` | **Portfolio 框架** (PR-1/PR-2/PR-3): `strategy.py` (`PortfolioStrategy` ABC + `PrecomputedScoreStrategy`) + `scoring.py` (`precompute_scores_from_legacy` — 把 per-stock `Strategy.generate_signals` 拼成 T×N score 面板) + `engine.py` (`PortfolioEngine` — top-K 选股 + 周期 rebalance + T+1 + 行业 cap 贪心 + eligibility 过滤 + **可插拔加权**:`equal`(默认,1/K,bit-exact)或 `mvo`,见 `weighting.py`) + `weighting.py` (**PR-P1-1**:`ledoit_wolf_cov`(纯 numpy LW 2004 收缩协方差)+ `solve_mvo`(scipy SLSQP box-constrained QP,`max μᵀw − (γ/2)wᵀΣw` s.t. `Σw=1, 0≤w≤w_max`,long-only)+ `compute_target_weights`(选中集 → `{code: weight}`,任何退化窗口/不可行 box/求解失败回退等权)。**无新依赖**(scipy 已在),仿 `RidgeWeighter` 纯 numpy 范式,不引 cvxpy/sklearn。μ=横截面 score(标准化),Σ=trailing `mvo_lookback` 日收益 LW 协方差) + `eligibility.py` (`EligibilityFilter` — 流动性/ST/min_history_bars;**2026-06-24 perf**:旧版每个 rebalance bar 对每票重 `pd.to_datetime`+全表过滤+tail(20) 聚合,O(rebalances×N×T)、100 票 engine 跑 38s 里 ~98% 在这。改为**每 panel 预算一次** as-of `(sorted_date_ns, eligible_bool/bar)` + 每 bar `searchsorted`,按 `id(panel_data)` 缓存。bit-exact(流动性用 `tail(20)` 新鲜窗口均值,非 online rolling)。100 票 eligibility 11.7s→0.09s,`PortfolioEngine.run` 38.2s→0.45s。universal win,所有策略受益) + `ensemble.py` (`StaggeredRunner` + `EnsembleResult` — N 个 `start_offset` 串行跑,聚合 min/p25/median/p75/max envelope + 等权均值 ensemble curve) + `result.py` (`PortfolioBacktestResult`/`PortfolioTrade`) + `report.py` (单 arm HTML + ensemble HTML;`render_portfolio_report` / `render_ensemble_report`)。PR-2 起 universe = `load_universe_cache`(`universe.parquet` 不存在时回退 cfg.stocks),`sector_map` 来自 `load_or_build_industry_map`,行业 cap "未知" 全空时跳过 cap、否则 Unknown 桶正常计。PR-3 起 CLI `staggered_starts > 1` 自动走 `StaggeredRunner`(串行,engine_factory 闭包保证每 offset fresh state);N=1 退化回单 arm 路径。无 portfolio AB (PR-4) |
...
| `src/stockpool/ipo_dates.py` | `code → IPO 日期` 映射,**多源**(2026-07-18):`source="auto"`(默认,baostock `query_stock_basic` → akshare 三张交易所名录 `stock_info_{sh,sz,bj}_name_code` fallback 链,与 industry_map 同范式)/ `"baostock"` / `"akshare"`;baostock 黑名单时 akshare 路径可离线重建。缓存 `data/ipo_dates.parquet` 30 天有效。当 `cfg.strategy.ml_factor.mask.enabled=true` 且 `cache_dir` 可用时,`MLFactorStrategy._get_ipo_dates` 自动加载并传给 `_listing_mask`,避免后者 `first_valid_index` 启发式把缓存窗口短的成熟股错认成新上市(panel union 早于该股缓存起点时会触发 bug,可使 mask=False 比例虚高到 50%)|
...
| `src/stockpool/backtesting/composite_weekly.py` | **(2026-06-24 perf)** `weekly_scores_by_bar` — `CompositeVerdictStrategy.generate_signals` 的周线打分快路径。旧实现每个 daily bar 都 `resample_to_weekly + add_all`(O(T²),~17s/票),改为"全量 resample + `add_all` 一次得到所有**已完成周**的精确指标 + 每 bar 只增量算**当前 partial week** 行 + 复用真实 `detect_signals`(4 行尾帧)"。**bit-exact**:EMA(`adjust=False`)递归一步外推 + rolling min/max 顺序无关均精确;⚠️ pandas `rolling.mean/std` 是 online(history-dependent)累加器,故 MA/BOLL/vol_ratio 仍对**完整 as-of 周线序列**跑 pandas rolling(闭式会在 1e-12~1e-15 量级翻转离散触发点)。~11× 加速,实测 30 票 0 mismatch。验证见 `scripts/verify_portfolio_ab_perf.py` + `tests/test_composite_weekly_fast.py`。`predict_latest`(单点日报)未改 |
...
- **`portfolio_backtest`** — Portfolio 级回测(PR-1 新增,默认 `enabled: false` 关闭)。`portfolio.{top_k=20, rebalance_n_days=5, max_per_industry=5, initial_cash=1.0, weighting="equal", mvo_risk_aversion=10.0, mvo_w_max=0.15, mvo_lookback=120, mvo_min_obs=20, hold_survivors=false, limit_guard=false}`（**2026-07-02 执行现实性**：`hold_survivors=true` 时留任股不再"卖出重买"——legacy 全清仓模型把换手钉死在 ~100%/rebalance，既高估成本又让换手敏感的 AB（rebalance 周期/horizon/MVO 低换手优势）全部失焦；新进股由被剔股的卖出款按权重吸收，权重在两次 rebalance 间漂移（接受）。`limit_guard=true` 时一字涨停开盘的候选在**选股阶段**被跳过（次名递补）+ fill 兜底拒单，一字跌停开盘的持仓**卖不出**、留到下个 rebalance；阈值按板块 `backtesting/limits.py`（ST 5% 不识别）。两开关默认 false = legacy bit-exact。**2026-07-18 修复**:①legacy+limit_guard 下留在 target 的一字跌停持仓不再被 buy 循环覆盖(旧 bug 会销毁其权益);②持仓股 close 缺失(停牌/退市)时 MTM/期末平仓按 `last_mark`(最后已知 close)而非 entry_price——含退市回填池的绝对数字自此变化(rv_eval 基线 Sharpe 0.331→0.288),历史数字不可直接比。**`rank_buffer_mult: float|None=null`(2026-07-18,opt-in)**:排名滞回带——留任股 rank 在 top(k×mult) 内保留席位,新入场仍需 top-k;降换手不拖慢入场。AB 实测 buffer=2.0 NOT CONFIRMED(+0.042 含 0)未 promote,机制可用}(**PR-P1-1 加权**:`weighting` 默认 `"equal"`=老的 1/K bit-exact;`"mvo"`=Ledoit-Wolf 协方差 + box-constrained MVO,`mvo_w_max` 必须 ≥ 1/top_k 否则该 rebalance 回退等权;只影响组合构造、**不进 score_cache_key** → equal/mvo 两 arm 共享 score panel 缓存,纯 engine-only AB) / `eligibility.{min_avg_amount_20d=5e7, exclude_st=true, min_history_bars=60, top_n_liquidity=null}`(PR-2 起 engine 实际读取;**`top_n_liquidity`(2026-07-02)**:每个 rebalance bar 按 as-of trailing-20d 成交额取 top-N 作为 point-in-time 流动性 universe,替代静态池文件(如 `top1000_liquid.parquet`)的期末成员前视——实测静态 top-1000 每天只覆盖真实 as-of top-1000 的中位数 326 只;null=不裁剪(legacy)) / `staggered_starts=1`(PR-3 起 `>1` 自动走 ensemble)/ `parallel_staggered: bool = False`(PR-T1.3,opt-in):N 个 staggered offset 并行跑(ProcessPoolExecutor)。要求 engine components 全部可 pickle;失败 fallback 串行 + warning。**实测加速**:single engine wall < 1s 的小 panel 反而变慢(Windows spawn + pickle 开销);1000 codes × 2500 bars 量级 ~1.9×;全市场 4000+ 票场景预期 2-3×。详见 `docs/superpowers/plans/2026-06-21-perf-tier1-decisions.md` #5。/ `score_cache_dir=data/portfolio_scores`(2026-06-25 起缓存键 = **scoring 签名** `score_cache_key`:仅打分相关配置 strategy+data + 打分 universe 的哈希,**不含** `portfolio_backtest`(top_k/rebalance/cap)→ 只调组合参数的 arm/重跑直接命中缓存、不重算昂贵的 walk-forward 打分;带 `v2` 版本前缀,旧缓存自动失效)/ **`universe_codes: list[str] | None = None`**(2026-05-31 新增,**解耦训练池与 portfolio universe**)。CLI `portfolio-backtest` 在 `enabled=false` 时退出 2;`--workers N`(2026-06-27 新增,默认 auto ≤3)透传给 `precompute_scores_from_legacy(n_workers=...)` 并行打分,大池子调高换 ~cores× 加速(每 worker pickle 整个 strategy ~6GB,`1` 强制串行省内存)——与 `portfolio-ab --workers` 同语义。PR-2 起:**训练池** universe 自动从 `data/universe.parquet` + `load_universe_cache` 装 4000+ 票(无该文件回退 cfg.stocks 并 log warning);`industry_map` 通过 `load_or_build_industry_map(source="auto")` 装载(首次跑 baostock 慢);name_map 来自 universe.parquet 的 `name` 列;cfg.stocks 自动 merge 到 pool(保证应用池始终可投资)。PR-3 起 `staggered_starts > 1` 自动跑 N 个 offset(`{0, 1, ..., N-1}`)各一份回测,聚合成 envelope + ensemble mean → `reports/portfolio/<date>.html` 出包络图 + per-offset 折叠卡。**`universe_codes`**:`None`(默认)= portfolio universe 同训练池(即全 pool_data,旧行为);设为显式 code 列表则 `precompute_scores_from_legacy` + `PortfolioEngine` 仅在该子集上运行(`factor_panel` build + ml_factor training_universe=all 仍用 **全** pool_data)。典型用法:训练池 4358 票 + portfolio universe 几十到几百票,避免 `precompute_scores_from_legacy` 在 4358 票全跑时 segfault(C 层 bug 不易排查)。两 arm 解耦后跑 ab_mask 实测 mask 提升 Δ Sharpe +0.04(详见 `docs/research/2026-05-31-a-share-quant-survey-comparison.md` 附录)
...
- `universe_delisted.parquet` — **退市股清单**(2026-07-02 幸存者偏差修复):`scripts/fetch_delisted_universe.py` 从沪深退市名单(akshare)+ 东财历史行情回填窗口内 253 只退市股的日线(244 只拉到数据,不复权、volume=手,与 mootdx 口径一致),bars 直接写 `data/<code>_daily.parquet` → 经 `load_universe_cache` 自动进入训练池/研究评估;**不**并入 `universe.parquet`(生产 name_map 保持干净)。Pool B 有 30 天新鲜度闸门防退市股混入日报推荐(`recommend_pool._MAX_STALE_DAYS`)
- `asof_top1000_union.parquet` / `rv_eval_pool.parquet` — 研究用:as-of top-1000 成员 union 统计 / 重验评估池(静态 top1000 ∪ 退市 244)
...
~1380 个,`pytest tests/ -q` 一次跑完(2026-07-18 起 0 skip:ops snapshot fixture 已重生成覆盖 GTJA191+全部新家族)。按域分布(节选):
...
| `test_composite_weekly_fast.py` | 周线快路径 `weekly_scores_by_bar` 与慢 per-bar 路径 bit-exact(多 seed 合成 + 短历史全 0)+ `generate_signals` 端到端 == live pipeline |
...
| `test_ml_weighter_sharpe.py` | SharpeWeighter: predictive 因子拿大权重 + sign inversion(负 Sharpe→负权重)+ L1-norm + `min_abs_sharpe` 过滤 + 零信号回退等权 + linear contributions 行和==predict + 缺 date MultiIndex raise + 薄横截面回退等权 + TwoStepPipeline 集成 + init 参数校验 |
| `test_ml_weighter_halflife_ic.py` | HalfLifeICWeighter: 短 halflife 偏好近期 regime 因子(early/late_good 设定)+ sign inversion + L1-norm + `min_abs_ic` 过滤 + 零信号回退等权 + linear contributions 行和==predict + 缺 date MultiIndex raise + 薄横截面回退 + TwoStepPipeline 集成 + init 参数校验 |
| `test_ml_weighter_ridge.py` | RidgeWeighter: 信号因子系数大于噪声 + L1-norm + alpha=0≈OLS / 大 alpha shrinks 系数 + sign preserved + linear contributions 行和==predict + TwoStepPipeline 集成 + 空特征矩阵 + init 参数校验 |
...
| `test_ipo_dates.py` | 多源 mock(baostock + akshare 三所拼表 / auto 链 fallback / akshare-only / 未知 source raise),cache hit / stale / force_refresh / failure-fallback / NaT 过滤 |
...
| `test_portfolio_engine.py` | PortfolioEngine:空面板 / 零成本恒价等权不变 / 现金守恒 / T+1 fill 在 open[t+1] / start_offset / 确定性 / rebalance diff / 末 bar 不执行 / 未知 code 过滤 / initial_cash 缩放 / **mvo 加权端到端跑通且与 equal 分叉 / mvo 冷启动回退等权** |
| `test_portfolio_weighting.py` | **PR-P1-1** `weighting.py`:Ledoit-Wolf 协方差(对称/PSD/少样本强收缩/单行退化)+ `solve_mvo`(box+simplex 约束 / 低风险厌恶偏高分 / 高风险厌恶趋等权 / 不可行 box 回退 / 非有限回退 / 单资产)+ `compute_target_weights`(equal 精确 / mvo 求和为1且在 box / 样本不足回退 / 缺失 code 补权重 / 未知 method raise) |
...
| `test_portfolio_eligibility_equiv.py` | 优化后的 `EligibilityFilter`(一次性预算 + searchsorted)与旧 per-bar 参考逻辑 bit-exact(3 seed × 3 config 跨多 date + 缺列) |
...
| `test_cli_portfolio_backtest.py` | `python -m stockpool portfolio-backtest`:smoke (composite_verdict 出 HTML) + `enabled=false` 退出 2 + `--refresh-scores` 旁路缓存 + `--workers` flag 透传 + `universe.parquet` 自动扩 universe + `staggered_starts=3` 出 ensemble HTML |
...
- **`layer_b_direct.py`** — **Layer B**(daily 截面 IC)显著性工具,**绕过 `run_portfolio_ab`**
  ...
  **⚠️ 2026-07-02 起默认 oracle 口径变更,新旧数字不可直接比**(复现旧口径加
  `--fwd-basis close --tradability none`,已验证 bit 级复现):默认 `--fwd-basis open`
  (open[t+1+h]/open[t+1]−1,与 label_basis=open + T+1 开盘成交对齐;旧 close 口径把拿不到的
  隔夜段计入每个分数)+ `--tradability entry`(入场 bar 一字涨停的 (day,code) 对剔除)。
  新增 `--asof-top-n N`(per-day PIT 流动性成员资格,基于全量已加载 universe 含退市回填)
  + `--date-start/--date-end`(时间 hold-out 窗口)。
- **`holdout_reselect.py`** — 因子池时间 hold-out 重选:复刻 P0-3 的 analyze+pick 流程但
  panel 截断到 `--cutoff`(默认 2020-12-31),产出 `reports/selection_holdout<year>.json`,
  与生产池在 cutoff 之后的窗口做 Layer B 对比 = 选池方法的真 OOS 检验。
- **`asof_membership_stats.py`** — 计算全 universe 的 as-of top-N 成员资格统计,写
  `data/asof_top<N>_union.parquet`。实测 15 年 union ≈ 4565/4841 只(几乎全市场都曾进过
  top-1000 流动性),静态池失真的直接证据。
...
- **F1 死锁 → 已转 fast-fail(2026-07-18)**:真实机制 = worker 被 OS OOM-kill 后
  `multiprocessing.Pool` **检测不到死 worker**,`imap_unordered` 永久阻塞(即"卡 1499/1500")。
  `scoring.py` 结果循环改 `IMapIterator.next(timeout=...)`(默认 1800s,env
  `STOCKPOOL_SCORE_RESULT_TIMEOUT_S` / 参数 `result_timeout_s` 可调),超时 terminate + 抛
  RuntimeError 提示 `--workers 1` 重试。健康路径不变;此前的 close/join 优雅收尾保留。
  hang 不再可能,但 OOM 本身仍需降 workers 规避(见 F3)。
...
- ~~`mask_exec` 未落地~~ **已落地(2026-07 更正)**:per-stock 双引擎(`BacktestEngine`/`MultiLotBacktestEngine` 的 `limit_pct` 参数,`backtest_runner` 生产注入)与 `PortfolioEngine`(`limit_guard`)都有开盘一字涨停拒买/跌停拒卖守卫,阈值按板块 `backtesting/limits.py`。残余缺口:`st_codes`(ST 5% 阈值)生产路径未传,ST 主板股按 10% 处理。
