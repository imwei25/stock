...
| `src/stockpool/ml/preprocess.py` | **截面预处理流水线** (winsorize / cs_zscore / industry_neutralize / **market_cap_neutralize**)。stateless 四函数 + `apply_preprocess_pipeline` 驱动 + `_is_all_off` short-circuit。`market_cap_neutralize_panel`(Phase 2)= 每日截面把因子对 log(总市值) 做 OLS 残差化(向量化,无 python 循环;size NaN 的票 pass-through,退化截面回退 demean)。baked 进 `build_factor_panel` cache,所有 callers 共享。industry/market_cap 两步均跳过 types 含 "fundamental" 的因子。**`symmetric_orthogonalize_panel`**(2026-06-10,默认关)= 流水线**最后一步**的 joint 跨因子变换:逐日截面对非基本面因子做对称(Löwdin)正交化 `F_orth = F_std · M^(-1/2)`(M = z-score 截面相关矩阵,`eigh` + 特征值 floor 1e-10),去除因子间相关性,order-independent;退化日(N_valid < K)passthrough,近奇异靠 floor 不爆 NaN,fundamental 因子跳过。与前四个 per-factor 步不同,它一次性吃整张 `{name: T×N}` 做关节变换。A/B 实测中性(P4-4,默认 false) |
| `src/stockpool/strategy_factory.py` | 按 `cfg.strategy.name` 工厂构造策略 + ML 通用 simulate;ml_factor 注入 `cache_dir` 以启用日报路径的月度训练缓存;`build_factor_panel` + `build_close_panel` 顶层助手用于 CLI 预算(close_panel 用于 `_try_fit` 跳过每 refit 重算因子);`load_or_build_factor_panel` 落盘缓存,key = (sorted factors + sorted codes + last_date + preprocess) sha256[:12],写 `data/factor_panels/<sig>/{manifest.json, close.parquet, <factor>.parquet × N}`;input 任一变化生成新 sig 重算。**Phase 2 市值中性**:`build_log_mcap_panel(pool_data, cache_dir)` 从 `data/mcap_shares.parquet`(最新 totalShare 快照,`scripts/pull_mcap_profit.py` 拉)算 `log(close × totalShare)`(shares 静态广播,close 仍日频 PIT);`maybe_inject_mcap_panel(preprocess_cfg, pool_data, cache_dir)` 在 `market_cap_neutralize=True` 时把该面板经 `factors.context.set_mcap_panel` 注入(prepare_pool / cli / ab.runner 三处 caller 已 wire,与 set_sector_map 并列);`build_factor_panel` 经 `get_mcap_panel()` 取回喂给 `apply_preprocess_pipeline`。**PR-3 性能**:`MLFactorStrategy._ensure_pooled_xy_long` 把全历史 stack 提到 `shared_cache["__pooled_xy_long__", sig]`,后续 refit 只做 `X.loc[:label_cutoff_ts]`(按 `label_end-1` 日期切,避免 horizon 行未来 label 泄露)+ per-stock tail,杜绝每次 refit 重 stack;`_try_fit` 用 `searchsorted` 早退跳过那些训练集明显太小的 cutoff,避免无意义的 slice+groupby。`stack_panel_to_xy` 内部改 numpy ravel + column_stack 替代逐因子 `.stack()`,~35× 加速 |
...
- **`preprocess.{winsorize, zscore, industry_neutralize, market_cap_neutralize, symmetric_orthogonalize, min_pool_size}`**(2026-06-06 Phase 1+1.5,2026-06-08 Phase 2,2026-06-10 正交化):截面预处理流水线。`winsorize`/`zscore` 默认 `null`/`false`(已按 P4-1b 验证默认开启:+0.245 sharpe / +2.64% return on 4358-票训练池);`industry_neutralize` 默认 `false`(单成员细分行业会触发 silent demean-to-zero,全市场参照设计前不推荐开启)。**`market_cap_neutralize`**(Phase 2,**默认 `true`**,P4-3 PASS):每日截面把因子对 log(总市值) 做 OLS 残差化,剥离规模暴露;mcap = `close × totalShare`(totalShare 取 `data/mcap_shares.parquet` 最新快照,静态广播,见 `scripts/pull_mcap_profit.py`)。需 caller 注入 log_mcap 面板(prepare_pool/cli/ab.runner 已 wire);**无 `data/mcap_shares.parquet` 时静默跳过 + warning**(所以缺数据不会报错,只是退化为不中性)。与 industry_neutralize 同属 ④ 中性化步,均跳过 types 含 "fundamental" 的因子。`min_pool_size: int = 200` runtime guard,n_codes < 200 时四步全跳 + warning(避免小池子退化)。baked 进 `factor_panels/<sig>/` cache(preprocess 入 hash);改 preprocess → 自动新 sig 重算。**A/B 结论**:P4-2 market_cap 全面优于 industry(Δsharpe +0.249);P4-3 market_cap 优于 no-neutralize base(Δsharpe +0.156,PASS)→ 默认开启。`industry_neutralize` 实测**有害**(与 selection 里的 `industry_relative_strength` 因子冲突),维持默认 false。对比配置 `ab_neutralize.yaml` / `ab_neutralize_confirm.yaml`。详见 `docs/ab_validation_results.md` P4-2/P4-3。**`symmetric_orthogonalize`**(2026-06-10,**默认 `false`**):流水线最后一步,对非基本面因子做逐日截面对称(Löwdin)正交化去相关(`F_std · M^(-1/2)`,见上 `preprocess.py` 行),之后 selector/weighter(IC/IR)照常跑在去相关后的因子上——无需改 pipeline。stateless per-day → look-ahead safe;入 `factor_panels/<sig>/` hash,翻转自动重算缓存。**A/B 结论(P4-4,NEUTRAL)**:在 winsorize+zscore+market_cap base 之上 Δsharpe mean **+0.007**(持平,未达 +0.05 门槛),改善回撤+中位指标但略降 mean return/胜率,净中性 → **默认保持关闭**。归因:`selection.json` 因子已经 `pick-by-ic`(max_corr 去相关)+ Lasso 筛过,冗余有限;换高相关原始因子或扩大应用池(16→50+)值得重测。对比配置 `ab_orthogonalize.yaml`(full)/ `ab_orthogonalize_small.yaml`(smoke)。详见 `docs/ab_validation_results.md` P4-4.
...
- `mcap_shares.parquet` — `scripts/pull_mcap_profit.py` 拉的全市场最新 `totalShare` 快照(cols: code/totalShare/pubDate/statDate,一股一行);`build_log_mcap_panel` 用其算 market_cap_neutralize 的 log(close × totalShare) 面板(shares 静态广播)。无有效期/手动刷新(baostock profit 表 ~0.7s/call 串行,全量 ~50-75 min)
...
| `test_factor_panel_cache.py` | `load_or_build_factor_panel` 落盘缓存:首次写 manifest + parquets / 二次命中不调 build_factor_panel / 改因子或股池触发新 sig / `refresh=True` 旁路 / 缓存内容与新建一致 / 空 pool 返回空 / preprocess sig 隔离 + 向后兼容 / **build_log_mcap_panel 静态广播 + 缺快照返 None / maybe_inject_mcap_panel disabled 时 no-op / build_factor_panel 经 context 取 mcap 面板** |
...
| `test_ml_preprocess.py` | 截面预处理 — 4 函数(winsorize / cs_zscore / industry_neutralize / **market_cap_neutralize**)+ `apply_preprocess_pipeline` 流水线(含 mcap 残差化 + fundamental 跳过 + missing-panel warning + orthogonalize-last + size-guard skip)+ `_is_all_off` short-circuit + size guard |
| `test_ml_preprocess_orthogonalize.py` | 对称(Löwdin)正交化:逐日输出 Gram 矩阵正交 / order-independent / 接近原因子 / 退化日 passthrough / NaN 跨因子传播 / fundamental 跳过 / 单因子不崩 / 近奇异 floor 保持 finite / 不 mutate 输入,9 个 case |
