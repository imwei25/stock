...
| `src/stockpool/ml/preprocess.py` | **截面预处理流水线** (winsorize / cs_zscore / industry_neutralize) — Phase 1。stateless 三函数 + `apply_preprocess_pipeline` 驱动 + `_is_all_off` short-circuit。baked 进 `build_factor_panel` cache,所有 callers 共享 |
...
- **`preprocess.{winsorize, zscore, industry_neutralize, min_pool_size}`**(2026-06-06, Phase 1+1.5):截面预处理流水线。`winsorize`/`zscore` 默认 `null`/`false`(可按 P4-1b 验证后开启:+0.245 sharpe / +2.64% return on 4358-票训练池);`industry_neutralize` 默认 `false`(单成员细分行业会触发 silent demean-to-zero,Phase 2 全市场参照设计前不推荐开启)。`min_pool_size: int = 200` runtime guard,n_codes < 200 时三步全跳 + warning(避免小池子退化)。baked 进 `factor_panels/<sig>/` cache(preprocess 入 hash);改 preprocess → 自动新 sig 重算。详见 `docs/ab_validation_results.md` P4-1b / `docs/superpowers/specs/2026-06-06-factor-preprocessing-phase1-design.md` Phase 1.5 amendments.
