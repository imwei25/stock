...
**因子库**总共 ~165 base names / ~280-320 variants(WQ101 全集 + 论文 B 9 家族精神复现 + EWMA + 基本面),通过双轴元数据组织,方便粗筛:

- **来源 (sources)**:`builtin`(10 个移植自老信号的技术因子)/ `wq101`(WorldQuant 101 Formulaic Alphas 全套)/ `paper_b`(论文 B arXiv 2507.07107 的 9 个家族精神复现)/ `ewma`(EWMA 平滑变体)/ `fundamental`(baostock 财务面板:PE/PB/ROE/营收增速等)
